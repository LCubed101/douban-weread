...
## 2026-08-27 — Real flomo AI Insight sample

### 12. flomo screenshot → Feishu Bot → multiple WeRead links

**Source**

A friend shared real screenshots generated by flomo AI Insight / book-list recommendations.

**Actual user goal**

The desired workflow is explicitly:

```text
flomo recommendation screenshot
        ↓
send screenshot to Feishu Bot
        ↓
recognize every book mentioned in the screenshot
        ↓
resolve each book independently
        ↓
return WeRead readable link / button for each matched book
```

This user does **not** require Douban in this route.

**Observed input characteristics**

The screenshots are not clean book lists. Book titles are embedded inside long explanatory paragraphs and grouped under themes. The same screenshot can contain many titles, including examples such as:
...
The titles are mostly explicitly delimited with Chinese book-title brackets `《》`, while surrounding prose contains many quoted phrases in `「」`. This matters because `《》` is a much stronger book signal than generic quotation marks.

**What this validates**

This is the first concrete sample strongly validating the V1.1 architecture:
...
WeRead matcher
  ↓
multiple “打开微信读书” actions
```

It also shows why the current image flow is insufficient: the existing OCR path attempts to infer one book from an image, while this real use case requires **many-book extraction from one screenshot**.

**Product / engineering implication**

- Keep `Book Mention Extractor` as the next core capability.
- Treat screenshot OCR as an input-normalization layer, not as the book resolver itself.
- For this first real sample, prioritize high-confidence `《...》` extraction after OCR before considering LLM extraction.
- Do not force the extracted books through Douban first; support a WeRead-only route.
- Return one result per extracted book, preferably as individual WeRead cards / buttons.
- Preserve OCR confidence / extraction uncertainty so the bot can fail closed instead of inventing titles.

**Suggested first acceptance case**

A single flomo screenshot containing 5+ explicit `《书名》` mentions should result in a de-duplicated list of recognized books and a WeRead lookup result for each one.
