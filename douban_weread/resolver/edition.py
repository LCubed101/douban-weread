...
_EDITION_MARKERS = {
    ...
    "revised": ("修订", "增订", "revised", "updated edition"),
    "annotated": ("注释版", "评注", "annotated"),
}
...
def compare_editions(source: Edition, candidate: Edition) -> EditionMatchResult:
    ...
    if not same_work:
        kind = MatchKind.DIFFERENT_WORK if title_similarity >= 0.88 and source_authors and candidate_authors else MatchKind.AMBIGUOUS
        return EditionMatchResult(
            candidate=candidate,
            score=score,
            kind=kind,
