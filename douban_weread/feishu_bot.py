...
def build_bot(
    *,
    ...
    async def on_message(message: InboundMessageLike) -> None:
        try:
            await _handle_message(channel, service, message)
...
async def _handle_message(
    ...
    if message.raw_content_type == "image":
        await channel.send(
            message.chat_id,
            {"text": "图片已经收到。下一步会接入飞书图片下载与书籍识别；现在先发书名或豆瓣链接。"},
            {"reply_to": message.message_id},
        )
        return
...
def main() -> None:
    ...
    print("No Douban or WeRead mutation is enabled in this milestone.")
    try:
